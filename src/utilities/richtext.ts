...
/** Plain text from a Lexical rich-text field (paragraphs joined with spaces). */
export function getRichTextPlainText(content: RichTextContent | null | undefined): string {
  if (!content?.root?.children?.length) return ''

  return content.root.children
    .map((node) => extractTextContent(node).trim())
    .filter(Boolean)
    .join(' ')
}
