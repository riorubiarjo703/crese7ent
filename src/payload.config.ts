...
import { LayoutBundles } from './collections/LayoutBundles'
...
export default buildConfig({
  ...
  db: mongooseAdapter({
    ...
    // These are defaults that work well for Serverless. Remove them for long running server processes.
    connectOptions: {
  ...
  collections: [Pages, LayoutBundles, Posts, Media, Categories, Users, Roles],
  ...
  plugins: [
    ...
    backupMongodbPlugin({
      access: (user) => checkRole(['admin'], user),
      seedDemoDumpUrl: `${NEXT_PUBLIC_SERVER_URL}/seed/demo-payblocks---demo-payblocks.trieb.work---1739813600714.json`,
    }),
    OAuth2Plugin({
