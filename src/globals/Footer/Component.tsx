...
export async function Footer({ publicContext }: { publicContext: PublicContextProps }) {
  const rawFooter = (await getCachedGlobal('footer', publicContext.locale, 2)()) as DataFromGlobalSlug<'footer'>
  const footer = await resolveIconsInData(rawFooter)
  ...
  switch (footerType) {
    ...
    case '9':
      return <Footer9 footer={footer} publicContext={publicContext} />
  }
