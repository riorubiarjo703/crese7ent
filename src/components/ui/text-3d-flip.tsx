...
const Text3DFlip = ({
  ...
  const runFlip = useCallback(
    async (toFlipped: boolean) => {
      if (isAnimatingRef.current || (persistFlip && hasFlippedRef.current && toFlipped)) return
      if (toFlipped === isFlippedRef.current) return
      ...
      try {
        ...
        const delays = Array.from({ length: totalChars }, (_, i) => {
          const index = toFlipped ? i : totalChars - 1 - i
          return getStaggerDelay(index, totalChars)
        })

        await animate(
          ".text-3d-flip-char",
          { transform: toFlipped ? rotationTransform : resetTransform },
          {
            ...transition,
    ...
    [animate, characters, getStaggerDelay, persistFlip, resetTransform, rotationTransform, transition],
  )
  ...
  const handleHoverEnd = useCallback(() => {
    if (persistFlip && hasFlippedRef.current) return
    void runFlip(false)
  }, [persistFlip, runFlip])
